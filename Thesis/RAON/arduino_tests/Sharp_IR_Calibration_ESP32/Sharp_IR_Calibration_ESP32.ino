/*
  Sharp_IR_Calibration_ESP32.ino

  ESP32 calibration sketch for Sharp GP2Y0A21YK0F analog IR sensors.
  Default pins:
    IR1 -> GPIO34 (ADC1)
    IR2 -> GPIO35 (ADC1)
...
#if defined(ESP32) && !defined(ADC_11db) && defined(ADC_ATTEN_DB_11)
#define ADC_11db ADC_ATTEN_DB_11
#endif

const unsigned long BAUD_RATE = 115200;
const int IR_PINS[2] = {34, 35};
...
void print_live_line() {
  ...
  Serial.print(ema1);
  Serial.print(" | IR2: ");
  Serial.print(blocked2 ? "BLOCKED" : "CLEAR");
  Serial.print(" raw=");
  Serial.print(raw2);
  Serial.print(" ema=");
  Serial.println(ema2);
...
void suggest_thresholds(bool apply_now) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (separation < 150) {
      Serial.println("  warning: low separation (<150 ADC). Adjust sensor angle/distance.");
    }
...
void process_command(String cmd) {
  ...
  if (upper.startsWith("SET ")) {
    ...
    if (sscanf(upper.c_str(), "SET %d %d %d", &sensor, &blocked_enter, &clear_exit) == 3) {
      ...
      if (blocked_enter < 0 || blocked_enter > ADC_MAX || clear_exit < 0 || clear_exit > ADC_MAX) {
        Serial.println("ERR thresholds out of ADC range");
        return;
      }
      int idx = sensor - 1;
...
void setup() {
  ...
  analogSetPinAttenuation(IR_PINS[0], ADC_11db);
  analogSetPinAttenuation(IR_PINS[1], ADC_11db);
#endif
  ...
  Serial.println("Sharp IR calibration test (ESP32) ready.");
  Serial.print("Pins: IR1=GPIO");
  Serial.print(IR_PINS[0]);
  Serial.print(", IR2=GPIO");
  Serial.println(IR_PINS[1]);
